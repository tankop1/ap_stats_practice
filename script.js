...
function ShowSecretPage()
{
    ...
    if (isValidPassword)
    {
        ...
        forceAnswerButton.css({'display': 'block'});
        SetupDownloadLinks();
    }
...
function GoBack()
{
    ...
    forceAnswerButton.css({'display': 'none'});
...
function SetForceAnswer()
{
    if (forceRightAnswer) forceRightAnswer = false;
    else forceRightAnswer = true;

    if (forceRightAnswer) forceAnswerButton.text('Force Right Answer: On');
    else forceAnswerButton.text('Force Right Answer: Off');
}
